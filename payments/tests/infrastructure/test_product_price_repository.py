...
def test_get_by_id(product_price_repo, product_price):
...
@pytest.mark.django_db
def test_get_by_id_not_found(product_price_repo):
    with pytest.raises(EntityNotFoundError):
        product_price_repo.get_by_id(9999)


@pytest.mark.django_db
def test_get_active(product_price_repo, product_price, product):
    inactive = ProductPrice(
        currency=Currency.EUR, price=Decimal("5.00"), product=product
    )
    inactive.set_active(False)
    product_price_repo.save(inactive)

    active = product_price_repo.get_active()
...
def test_get_active_pagination_limit_and_offset(
    ...
    first_page = product_price_repo.get_active(limit=2, offset=0)
    second_page = product_price_repo.get_active(limit=2, offset=2)
...
@pytest.mark.django_db
def test_get_active_by_product_id(product_price_repo, product_price):
    assert product_price.product.id is not None
    loaded = product_price_repo.get_active_by_product_id(product_price.product.id)
    assert loaded.id == product_price.id
...
def test_get_active_by_product_id_not_found(
    ...
    with pytest.raises(EntityNotFoundError):
        product_price_repo.get_active_by_product_id(inactive_product.id)
...
def test_get_active_by_product_ids_filters_by_currency(
    ...
    prices = product_price_repo.get_active_by_product_ids(
        [product.id],
...
def test_get_active_by_product_ids_returns_active_only(
    ...
    prices = product_price_repo.get_active_by_product_ids([product.id])
...
def test_get_active_by_product_ids_multiple_products(
    ...
    product_price_repo.save(second_price)
...
@pytest.mark.django_db
def test_save_create_assigns_id(product_price_repo, product):
